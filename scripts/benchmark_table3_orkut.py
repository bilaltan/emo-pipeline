"""
Benchmark Table 3 Reproduction & Comparison: Orkut Dataset (117.19M edges)

Directly compares against Table 3 from the paper:
- Upper Bound (Full Graph): 0.8520
- Stage 3a only:            0.7850
- LakeGRL (Paper):          0.8120
...
def load_orkut(sample_edges=100000):
    print("Loading Orkut graph from data/orkut/com-orkut.ungraph.txt.gz...")
    t0 = time.time()
...
def run_orkut_benchmark():
    n_nodes, feat_t, all_src, all_dst = load_orkut(sample_edges=100000)
    ...
    print(f"  ✓ Stage 3a Simulation ROC-AUC: {auc_stage3a:.4f} (Paper Table 3 Stage 3a: 0.7850) in {time.time()-t0:.1f}s")

    # 2. EVALUATE NEW DEVELOPMENT
    ...
    # 3. PRINT COMPARISON
    paper_upper = 0.8520
    paper_stage3a = 0.7850
    paper_lakegrl = 0.8120
    
    print("\n" + "="*78)
    print("      TABLE 3 (ORKUT) LINK PREDICTION PERFORMANCE COMPARISON")
    print("="*78)

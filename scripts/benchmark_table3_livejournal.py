"""
Benchmark Table 3 Reproduction & Comparison: LiveJournal Dataset (34.68M edges)

Directly compares against Table 3 from the paper:
- Upper Bound (Full Graph): 0.8840
- Stage 3a only:            0.8120
- LakeGRL (Paper):          0.8302
...
def load_livejournal(sample_edges=100000):
    print("Loading LiveJournal graph from data/livejournal/com-lj.ungraph.txt.gz...")
    t0 = time.time()
    gz_path = "data/livejournal/com-lj.ungraph.txt.gz"
    
    # Read edges in chunks to keep memory low and fast
    chunks = []
    chunk_sz = 200000
    total_read = 0
    with gzip.open(gz_path, 'rt') as f:
        ...
        for c in reader:
            chunks.append(c)
            total_read += len(c)
            if total_read >= sample_edges * 4: # read sufficient edges for sub-graph density
                break
    ...
    print(f"  ✓ Extracted sub-graph: {n_nodes:,} nodes, {len(src):,} edges in {time.time()-t0:.1f}s")
    
    # Generate 128-dim node features using standard high-dimensional random projection
    np.random.seed(42)
    torch.manual_seed(42)
    feat_dim = 128
    node_feats = np.random.randn(n_nodes, feat_dim).astype(np.float32)
    # Add degree-correlated structural signal
    deg = np.bincount(src, minlength=n_nodes) + np.bincount(dst, minlength=n_nodes)
    log_deg = np.log1p(deg)[:, None]
    node_feats[:, :16] += log_deg * 0.5
    node_feats = node_feats / (np.linalg.norm(node_feats, axis=1, keepdims=True) + 1e-8)
    
    return n_nodes, torch.tensor(node_feats, dtype=torch.float32), src, dst
...
def run_livejournal_benchmark():
    n_nodes, feat_t, all_src, all_dst = load_livejournal(sample_edges=100000)
    
    # Forward edges only
    mask = all_src < all_dst
    fwd_src = all_src[mask]
    fwd_dst = all_dst[mask]
    
    perm = np.random.permutation(len(fwd_src))
    sample_sz = min(60000, len(fwd_src))
    sampled_idx = perm[:sample_sz]
    
    n_tr = int(0.8 * sample_sz)
    train_src = fwd_src[sampled_idx[:n_tr]]
    train_dst = fwd_dst[sampled_idx[:n_tr]]
    test_src = fwd_src[sampled_idx[n_tr:]]
    test_dst = fwd_dst[sampled_idx[n_tr:]]
    
    print(f"  Benchmark edges: {len(train_src):,} train, {len(test_src):,} test pairs.")
    
    # Partition using Louvain
    print("  Partitioning with Louvain...")
    t_part = time.time()
    G = nx.Graph()
    G.add_nodes_from(range(n_nodes))
    for s, d in zip(train_src[:30000], train_dst[:30000]):
        G.add_edge(s, d)
    ...
    for cid, comm in enumerate(comms):
        for u in comm:
            node_to_comm[u] = cid
    print(f"  ✓ {len(comms)} communities partitioned in {time.time()-t_part:.1f}s.")
    
    src_comm = np.array([node_to_comm.get(u, -1) for u in train_src])
    dst_comm = np.array([node_to_comm.get(v, -2) for v in train_dst])
    intra_mask = (src_comm == dst_comm) & (src_comm != -1)
    cut_mask = ~intra_mask
    print(f"  Train: {intra_mask.sum():,} intra-community ({intra_mask.mean()*100:.1f}%), {cut_mask.sum():,} boundary cut ({cut_mask.mean()*100:.1f}%).")

    # 1. EVALUATE STAGE 3a (Original Pipeline: Severed Boundaries, Hadamard, Naive Negatives)
    ...
    with torch.no_grad():
        ...
        auc_stage3a = roc_auc_score(y_true, y_sc)
        
    print(f"  ✓ Stage 3a Simulation ROC-AUC: {auc_stage3a:.4f} (Paper Table 3 Stage 3a: 0.8120) in {time.time()-t0:.1f}s")

    # 2. EVALUATE NEW DEVELOPMENT (4-Way Siamese Decoder, Rejection Negatives, 1-Hop Boundary Halo Expansion)
    ...
    # 3. PRINT COMPARISON
    paper_upper = 0.8840
    paper_stage3a = 0.8120
    paper_lakegrl = 0.8302
    
    print("\n" + "="*78)
    print("   TABLE 3 (LIVEJOURNAL) LINK PREDICTION PERFORMANCE COMPARISON")
    print("="*78)
